...
`profiles`: `username`, `displayName`, `avatarUrl?`, `bio?` (≤160), `website?`, `x?` (canonical handle, no `@`), `github?`, `linkedin?`, `location?` (≤60), `profilePublic?` (default true), `handleConfirmed?` (default true — `false` only on a row minted by `profiles.ensure`, see below), `xUserId?` / `xConnectedAt?` (set only by X OAuth), `xFollowers?` / `xFollowersAt?` (X follower count read from the founder's own token — Connect X or X sign-in — refreshed daily, see `docs/SOCIAL.md`), `socialPrefs?`, `followerCount?` (UserTrack followers), `feedSeenAt?` (last visit of `/app/following`, `docs/FOLLOWS.md`), `attribution?` (first-touch sign-up source `{ ref?, source?, medium?, campaign?, at }`, written once on insert, never public — `docs/ANALYTICS.md`), `_creationTime` → "Joined".

Only `username` and `displayName` are required. Onboarding asks for name, handle, optional avatar URL and optional X handle — as the **last** step, right before Publish (A234).

**`profiles.ensure`** (idempotent mutation, called once when the wizard mounts) creates that row up front, because `saas.ownerId` has to point somewhere before the founder has picked anything. The handle is derived server-side — OAuth name → email local part → `founder`, `slugify`d, padded to the 3-character minimum, capped at 24 characters, never a `RESERVED` word, collisions resolved with a numeric suffix like `uniqueSlug` — and a pending `profilePrefills` row (GitHub / X sign-in) is consumed onto it. The row carries `handleConfirmed: false`, and `isProfileVisible` (`convex/domain/visibility.ts`) keeps it out of `/u/<handle>`, its OG image and card, `/api/v1/users/*`, search, the sitemap, the owner block of public project pages and boards, follows and the MCP gateway until `profiles.upsert` sets `handleConfirmed: true`. Nothing reaps abandoned placeholders (A235).
...
- **`enrich.avatar({ x?, github?, includeEmail? })`** — resolves a profile picture from the handles the founder already gave us, trying free sources in order and reporting which one answered (`convex/lib/avatarSources.ts`, unit-tested): X's own API when the plan allows `users/by/username`, else `unavatar.io/x/<handle>`; then `github.com/<handle>.png`; then Gravatar by `SHA-256` of the account email. **Gravatar is only tried with `includeEmail`**, which the UI sets on an explicit **Fetch** click and never on the automatic pull — a typed handle is intent, the account address is not. Typing a handle fills an empty avatar once per handle/GitHub combination; the automatic attempt is silent on failure. unavatar's free tier is only **25 lookups/day per IP**, i.e. per deployment, which is exactly why GitHub and Gravatar are in the chain rather than a nicety (`HUMAN_TODO.md` → ONB-1).
- **Images are copied into Convex storage**, validated by the same rules as an upload (`convex/lib/uploads.ts`: ≤1 MB, PNG / JPG / WebP), so a pulled avatar or favicon survives the origin changing it. Every lookup runs server-side, so the founder's IP and user agent never reach these services — see `/privacy` section 3.11. Icons we cannot fetch and store are not offered at all rather than hot-linked, which is why the icon ranking prefers a raster `apple-touch-icon` over an SVG or `.ico`.
