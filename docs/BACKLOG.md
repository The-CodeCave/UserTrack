...
- ☑ **ANALYTICS-1** — self-hosted cookieless Rybbit: `<AnalyticsScript/>` + `<AnalyticsIdentity/>` in the root layout (skip / mask patterns, env-configurable host + site id, off in tests), typed 43-event catalog in `src/lib/analytics.ts` wired at every call site, `serverTrack()` from route handlers (`api_request`, `mcp_tool_called`, `badge_rendered`, `embed_rendered`, `native_event_ingested`) and `convex/lib/analytics.ts` (`webhook_delivered`, `sync_completed`, opt-in per deployment), privacy wording aligned, `docs/ANALYTICS.md` with the goals / funnels to create by hand.
...
- ☐ **OPS-2** — public-page caching + bounded board queries.
- ☐ **OPS-3** — retention, Sentry, error boundaries, `/api/health`, CI, doc drift.
- ☐ **SHIP-1** — consolidation + launch checklist.
