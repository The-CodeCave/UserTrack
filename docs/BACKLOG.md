...
- ☐ **Move the remaining two-phase accumulators to a scratch table** — `daily.benchmarks` and `daily.snapshotRankings` still hold their projection in the action's memory, which is comfortable to roughly 50k projects (`daily.snapshotRankings` keeps whole rows and is the first to feel it). `leaderboard.rerank` was converted in FIX-3; past ~5,000 products its phase 2 needs paging too (A219).


## v1.0.1 — Post-loop review fixes

### Epic 96 — Review findings (P0)
- ☑ **SHIP-2** — `apps/waitlist/` merged into `main` as a separate deployable (`apps` excluded from the root `tsconfig.json` / `eslint.config.mjs`, which the merge proved necessary), full clean verification (lint 0 errors, typecheck, 666 tests / 84 files, packages 61, build, smoke, header / health / rate-limit transcript, screenshots at 375 / 768 / 1440), `docs/RELEASE-v1.0.md` *v1.0.1 review fixes* section, README / DEPLOYMENT / CODECRAFT / ASSUMPTIONS / HUMAN_TODO updated, `main` pushed. Not deployed.
- ☑ **FIX-4** — durable Better Auth rate limit (`authRateLimits` table + `rateLimit.customStorage.consume` as one Convex mutation; `/sign-in/*` 20 per 10 min, `/sign-up/*` 10 per hour, verification / password-reset mail 5 per 10 min, else 100/min), keyed on `x-ut-client-ip` which the `/api/auth` proxy stamps from the FIX-1 trust rules; `next.config.ts` header ordering pinned by a test that uses Next's own `getPathMatch`; `isPrivateIp` normalises every IPv6 form before the range check; `trustmrr.status` requires a founder profile. AC: `convex/authRateLimits.test.ts`, `convex/lib/ssrf.test.ts`, `convex/trustmrr.test.ts`, `src/lib/security-headers.test.ts`. Done.
- ☑ **FIX-3** — `leaderboard.rerank` converted from one `internalAction` to a three-phase scheduler chain of mutations over the new `rankScratch` table (byte-identical ranking, same `runId` lock, same `pages`/`items` bookkeeping, leftovers of a dead chain swept), and `public.landing` rewritten from the unbounded `publicSet` + per-row `dailyMetrics.collect()` to two bounded `boardRows` walks + `publicStats` (~430 read documents on a 1,000-product seed), `sparkline` capped at `take(30)`, `boardRows` treating a missing index key as the lowest score. AC: `convex/jobs.test.ts`, `convex/boardScan.test.ts`, screenshots `docs/screenshots/v1/FIX-3`. Done.
- ☑ **FIX-2** — `jobRuns` running lock with a per-job TTL (`startRun` returns `null` while the previous chain is open; stale rows closed as `abandoned (lock expired)`), applied to every paged driver incl. the new `follower refresh` run, driver bodies wrapped so a throwing page releases the lock, `running` / `stale` on `/api/health?deep=1`. AC: `convex/jobs.test.ts`, `convex/emailFunctions.test.ts`, `src/app/api/health/route.test.ts`. Done.
