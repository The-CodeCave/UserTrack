...
| **Better Auth sign-in / sign-up is not rate limited** | Better Auth runs inside the Convex HTTP router behind the `/api/auth` proxy, so the Next.js `limit()` helper cannot wrap it and the client IP does not reach Convex untouched. Deferred from SEC-2 and still open in `docs/BACKLOG.md`. | Needs a `hooks.before` in `convex/auth.ts` that trusts a proxy-forwarded IP header. Email verification (SEC-1) already blocks the value of mass sign-ups. |
