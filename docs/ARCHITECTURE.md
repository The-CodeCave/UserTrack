...
Score = provider base (auth incl. Better Auth native plugin 40 · analytics/endpoint 30 · foreign endpoint 10 · manual 5) + connection age (≤25 over 30 days) + sync continuity (≤20) + activation data (5) − open flags (high 15 / medium 8 / low 3).
...
## Better Auth native integration (v0.6)

`packages/better-auth` is an independently published npm package (`@usertrack/better-auth`, ESM, peer `better-auth >=1.3`) that adds `POST <basePath>/usertrack/metrics` to the founder's Better Auth instance and `databaseHooks.user.create/delete.after` → signed lifecycle events. It never imports from the app. UserTrack's side: provider `convex/providers/betterAuth.ts` (signed pull, response verification bound to the request nonce, protocol/plugin version gate, actionable error mapping), `convex/betterAuth.ts` (credential lifecycle: `ut_int_` secret shown once, stored in `integrations.config` with SHA-256 + prefix; `awaitingVerification` until the first successful verify; `ingestEvent` action for pushed events → `integrationEvents`, deduped by `eventId`, pruned after 30 days), `integrations.verifyStored` / `markVerified` (owner verify → first sync), Next route `/api/integrations/better-auth/events` (thin proxy, rate-limited per project), `convex/lib/betterAuthSetup.ts` (install plan shared by dashboard, `/developers/integrations/better-auth` and the MCP tools `usertrack_get_better_auth_setup` / `usertrack_create_integration`). Protocol v1 is implemented twice (package + `convex/lib/betterAuthProtocol.ts`) and frozen by shared fixtures. Sync, trust, funnel, public API and provenance treat it as any other verified `users` provider (base 40, "Verified via Better Auth").
