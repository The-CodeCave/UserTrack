...
- **Client IP trust model** (`src/lib/client-ip.ts`): Railway's edge *appends* the real client address to `x-forwarded-for`, so only the **last** entry is proxy-set and every entry left of it is attacker-controlled. `clientIp()` takes the last hop, validates it as an IPv4/IPv6 literal, then falls back to `x-real-ip`, then `"unknown"`. Nothing keys a limit on the first hop anymore. Once **Cloudflare** proxies the app (orange cloud) that last hop is Cloudflare's own address and every visitor would share one bucket, so `clientIp()` reads `cf-connecting-ip` then `true-client-ip` **first** — but only when `UT_TRUST_CF_HEADERS=1` (default off; both headers are trivially forged by anyone reaching the origin directly). Set it on Railway at the same moment the proxy is enabled, never before (`HUMAN_TODO.md` step 12).
...
## Public surface
- **Pages** read Convex through the cached public read path (`revalidate = 300`; ISR shell or per-request render + cached data — see *Public caching* below). Product pages show the funnel (timeframe tabs), the trending ⓘ explanation and the public benchmark statement; `/discover` renders the sections + feed (`discovery-feed.tsx`).
...
### URL-first entry (`/preview`, v1.0.2)
The landing hero is a URL field, not a sign-up button: the visitor sees their own page before an account exists.
- `POST /api/preview` owns the per-IP limit (`preview`: 10/h, `convex/lib/rateLimits.ts`) and passes `UT_GATEWAY_SECRET` — the same shape as `/api/feedback`. No new env var.
- `enrich.previewSite` is `enrich.site` without the account: `requireGateway` instead of `budget`, the same `publicUrl` SSRF guard, the same manual redirect following, timeouts and 512 KB cap (`readSite` is shared). **It writes nothing** — no DB row, no `ctx.storage`; the logo comes back as the origin's own absolute URL. Its only read is `enrich.claimedByDomain`, a bounded walk of the public index that reports whether the domain is already a public page (`{ slug, name }` → the preview links to `/s/<slug>`).
- `detectSiteHints` (`convex/lib/siteMeta.ts`, pure + unit-tested) maps the founder's own HTML onto the wizard's `StackChoices`. Signals are **asset hosts (`<script>`/`<link>`/`<iframe>`), inline `<script>` globals and outbound store links only** — never prose, so "we use Stripe" in a paragraph proves nothing. First table hit per key wins; the category is guessed only when exactly one hint phrase matches, else `undefined`.
- `/preview` is `robots: { index: false }`, has no `?url=` sharing semantics (the address travels in `sessionStorage["ut:preview-draft"]`, schema- and age-checked on read, `src/lib/preview-draft.ts`) and shows **only real public data**: the scraped identity, the detected stack as sentences, and category peers from `public.board` + counters from `public.stats`. Metric slots render empty with the reason. Inventing or blurring a visitor's numbers would contradict the one thing the product sells.
...
- **Route config, and which routes are actually ISR.** `export const revalidate = 300` is on every public page, OG image and `sitemap.ts`, but only some of them end up in the route cache (`.next/prerender-manifest.json`, and the `○` / `●` marks in the build output):
  - **ISR — a cached HTML shell, revalidated after 300 s:** `/`, `/categories`, `/rankings`, `/sitemap.xml`, `/robots.txt`, the static OG images, and the on-demand-rendered `/s/[slug]`, `/s/[slug]/share/[kind]`, `/u/[username]`. Those three also export `generateStaticParams = async () => []` — Next 16 leaves a dynamic segment out of the ISR manifest entirely without it (A195).
  - **Per-request render + cached data + CDN:** every route that awaits `searchParams` — all board pages (`/leaderboard`, `/trending`, `/new-saas`, `/hidden-gems`, `/biggest-movers`, `/most-new-users`, `/best-*`, `/fastest-growing-*`), `/discover`, `/compare`, `/stacks/[slug]`, `/categories/[category]`, `/rankings/[year]/[month]/[category]`. Awaiting `searchParams` opts the route out of the route cache, so the HTML is rendered on every request; what makes them cheap is `cachedQuery` (the Convex read, `unstable_cache`, 300 s per URL) plus a CDN honouring the `Cache-Control` below. Their `revalidate` export is inert — Next 16 does not warn about it and it is left in place (A225). **A CDN cache rule is therefore required, not optional** (`HUMAN_TODO.md` step 14): without it a crawler hit re-renders the page every time.
...
- Onboarding: manual path Your SaaS → Platform → Stack → Source → Activation (optional) → Conversion (optional) → Profile → Publish; AI path Set up with AI → Profile → Live. **Profile is last** (A234): `profiles.ensure` mints the row `saas.ownerId` needs with a derived handle and `handleConfirmed: false`, which keeps it out of every public read until the founder confirms it one step before Publish (`profiles.upsert`); `completeOnboarding` refuses while it is unconfirmed. Steps the `/preview` reading already answered arrive prefilled and highlighted (`ut:preview-draft`, see *URL-first entry*), so a founder who came through the landing field can land on **Source** with the provider preselected — never silently: every prefilled field is ringed and editable.
