...
- Onboarding: manual path Profile → Your SaaS → Platform → Stack → Source → Activation (optional) → Conversion (optional) → Publish; AI path Profile → Set up with AI → Live. Steps the `/preview` reading already answered arrive prefilled and highlighted (`ut:preview-draft`, see *URL-first entry*), so a founder who came through the landing field can land on **Source** with the provider preselected — never silently: every prefilled field is ringed and editable.
