...
| Scope | Grants | Tools |
| --- | --- | --- |
| `profile:read` | Founder profile and account summary | `usertrack_get_account`, `usertrack_get_profile`, `usertrack_get_founder_url`, `usertrack_export_account` |
...
| `projects:write` | Create projects, edit metadata, publish, import a profile from TrustMRR. Never deletes. | `usertrack_create_project`, `usertrack_update_project`, `usertrack_import_from_trustmrr` |
...
52 tools: the 15 from v0.3, 8 added in v0.4 (marked **v0.4**), 4 added in v0.5 (marked **v0.5**: conversion setup, identity mapping, funnel history, cohorts), 2 added in v0.6 (marked **v0.6**: native credential creation + the Better Auth setup alias), 1 added in v0.7 (marked **v0.7**: `usertrack_get_native_setup` for every SDK source), 6 added in v0.8 (marked **v0.8**: founder profile, share events, share cards, X drafts, founder URL) and 14 added in v0.9 (marked **v0.9**: discovery, watchlist, rank + benchmark history, datasets, webhooks) and 2 added in v1.0 (marked **v1.0**: `usertrack_export_account`, `usertrack_import_from_trustmrr`). Every tool takes a project reference `{ projectId?: string, slug?: string }` where noted (`ref`); either is accepted and ownership is enforced on both. All tools are annotated `idempotentHint: true`, `destructiveHint: false`, `readOnlyHint` per tool. Results are returned as JSON text plus `structuredContent`.
...
| `usertrack_update_project` | `projects:write` | write | `ref`, `name?` (≤100), `description?` (≤500), `websiteUrl?`, `category?`, `tags?`, `logoUrl?`, `newSlug?`, `isPublic?`, **product profile (PROFILE-1)**: `foundedAt?` (`YYYY-MM`), `markets?[]` (≤5 curated), `techStack?[]` (≤20 curated slugs / labels or free text), `marketingChannels?[]` (≤15 curated), `cofounders?[]` (≤5 × `{name?, x?, github?}`), `country?` (ISO-2), `funding?` (`bootstrapped` / `vc`), `teamSize?` (`1` … `50+`), `valueProposition?` (≤300), `problemSolved?` (≤300), `audience?` (≤200), `pricingSummary?` (≤300, the model in words — never numbers), `additionalInfo?` (≤500), `anonymous?`, `hideFromSearch?` | `updated[]` (changed fields), `project` (summary incl. every profile field). `isPublic: true` publishes and triggers a rerank. Unknown market / channel slugs are dropped, invalid cofounder handles and country codes are `bad_request`. No revenue fields exist. |
| `usertrack_import_from_trustmrr` **v1.0** | `projects:write` | write | `urlOrSlug` (`https://trustmrr.com/startup/<slug>` or slug), `ref?`, `apply?`, `overwrite?` | `prefill` (name, description, websiteUrl, logoUrl, category, markets, techStack, marketingChannels, cofounders, country, funding, teamSize, foundedAt, valueProposition, problemSolved, audience, pricingSummary, projectType — never a revenue field), `unmapped[]` (`stack: …` / `channel: …` / `market: …` values that matched nothing), `source {slug, url}`, `applied` (`null` for a preview; with `ref` + `apply: true` → `{ applied[], project }` after filling the project's empty fields — `overwrite: true` replaces — through the same path as `usertrack_update_project`, plus `trustmrrSlug` for the "Also on TrustMRR" link; audit `import_from_trustmrr`). Errors: `not_configured` (no operator key on this deployment — do not retry), `bad_request` (unparseable ref / `apply` without ref), `not_found`, `rate_limited` (5 per founder per 10 min, or TrustMRR's own limit), `upstream`. |
...
| `usertrack_get_profile` **v0.8** | `profile:read` | read | — | `profile` (name, username, bio, links, canonical X handle, `xState`: `unavailable` · `handle_provided` · `connected_via_oauth`, `xFollowers?` / `xFollowersAt?` — read from the founder's own connected X account, v1.0, location, `profilePublic`, `socialPrefs`), `aggregates` (Σ users, new users 7d / 30d, combined growth %, change vs previous 30d, **weighted** activation rate + contributing projects, published converted users, best rank, trending count, biggest-growth project), `projects[]`, `urls` (profile, card, api, settings), `formulas`. Public projects only. |
| `usertrack_update_profile` **v0.8** | `profile:write` | write | `displayName?`, `bio?` (≤160), `website?` (https), `x?` (`@name` / `name` / x.com URL → canonical), `github?`, `linkedin?`, `location?`, `avatarUrl?` (https), `profilePublic?` | `updated[]` + the same payload as `usertrack_get_profile`. Never touches the username or tokens. Making a hidden profile public again (`profilePublic: true`) needs a verified email — `forbidden` otherwise, like the dashboard. Audit `update_profile`. |
...
| `usertrack_export_account` **v1.0** | `profile:read` | read | — | GDPR Art. 20 export, identical to the Settings download: `format`, `exportedAt`, `account`, `profile`, `projects[]` (summary + `visibility`, `integrations[]` with `publicConfig` only, `milestones[]`), `follows[]`, `webhookEndpoints[]` (`secretMasked`), `developerTokens[]` (`token` masked), `emailPreferences`, `socialConnections[]` (no tokens). Account **deletion is not available through MCP** — only from `/app/settings`. |
