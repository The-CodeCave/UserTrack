...
| `metrics:read` | Metrics, history, ranks, milestones, funnel, trending, benchmarks, compare, share cards, embeds | `usertrack_get_metrics`, `usertrack_get_growth_history`, `usertrack_get_rank`, `usertrack_get_milestones`, `usertrack_get_share_url`, `usertrack_get_funnel`, `usertrack_get_trending`, `usertrack_get_benchmark`, `usertrack_compare_projects`, `usertrack_get_share_card`, `usertrack_get_embed_code`, `usertrack_get_funnel_history`, `usertrack_get_cohorts` |
...
29 tools: the 15 from v0.3, 8 added in v0.4 (marked **v0.4**), 4 added in v0.5 (marked **v0.5**: conversion setup, identity mapping, funnel history, cohorts) and 2 added in v0.6 (marked **v0.6**: Better Auth native plugin setup + credential creation). Every tool takes a project reference `{ projectId?: string, slug?: string }` where noted (`ref`); either is accepted and ownership is enforced on both. All tools are annotated `idempotentHint: true`, `destructiveHint: false`, `readOnlyHint` per tool. Results are returned as JSON text plus `structuredContent`.
...
| `usertrack_get_better_auth_setup` **v0.6** | `integrations:read` | read | `ref?`, `packageManager?` (`npm` / `pnpm` / `yarn` / `bun`, from the lockfile), `betterAuthVersion?`, `framework?`, `authConfigPath?` | Structured install plan for `@usertrack/better-auth`: `installCommand` (+ `installCommands` for all four managers), `configExample` (import + append `userTrack()` to the existing `plugins` array), `environmentVariables` (`USERTRACK_PROJECT_ID`, `USERTRACK_SECRET`), `envExample`, `codeModificationRules` (preserve every option, never replace the plugins array, never hardcode/commit the secret, typecheck, deploy), `whatIsSent`, `endpoint`, `supported` + `unsupportedReason` (better-auth < 1.3), `steps[]`, `verification`, `nextTool`; with `ref`: `project`, `integration` (state, `awaitingVerification`, `pluginVersion`). Never contains the secret. |
| `usertrack_create_integration` **v0.6** | `integrations:write` | write | `ref`, `provider: "better_auth"`, `url?` (Better Auth base URL, default `<websiteUrl>/api/auth`), `rotate?` | Creates the native integration and returns the credential **once**: `projectId`, `secret` (`ut_int_…`), `env` (ready-to-paste lines), `metricsUrl`, `integration` (secret-free view), `nextTool`. Idempotent: an existing Better Auth source returns `created: false, secret: null` (rotate with `rotate: true`, which invalidates the previous secret). Audit action `create_integration` (prefix only, never the secret). |
...
- Provider priority for the `users` role (`recommendIntegrations`): **Better Auth (official plugin) → Supabase → Clerk → Firebase → Auth0 → PostgreSQL → JSON endpoint**; `manual` is never recommended. Direct auth providers come first (read-only key, no code change), then a read-only database connection (no code change either), then the universal endpoint. When nothing supported is detected, the recommendation is a small JSON endpoint on the product's own domain.
...
- For **Better Auth** the agent edits the repo: `usertrack_create_integration` (secret returned once → local env + hosting env, never committed) → `usertrack_get_better_auth_setup` → install `@usertrack/better-auth`, append `userTrack({ projectId: process.env.USERTRACK_PROJECT_ID!, secret: process.env.USERTRACK_SECRET! })` to the existing `plugins` array, add both names to `.env.example`, typecheck, deploy → `usertrack_verify_integration` (404 = plugin not deployed, 401 = env mismatch, both with hints) → `usertrack_sync_project`. Idempotent: re-running create returns the existing integration without a secret; verify marks a pending integration as connected and starts syncing.
