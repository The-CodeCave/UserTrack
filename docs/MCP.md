...
30 tools: the 15 from v0.3, 8 added in v0.4 (marked **v0.4**), 4 added in v0.5 (marked **v0.5**: conversion setup, identity mapping, funnel history, cohorts), 2 added in v0.6 (marked **v0.6**: native credential creation + the Better Auth setup alias) and 1 added in v0.7 (marked **v0.7**: `usertrack_get_native_setup` for every SDK source). Every tool takes a project reference `{ projectId?: string, slug?: string }` where noted (`ref`); either is accepted and ownership is enforced on both. All tools are annotated `idempotentHint: true`, `destructiveHint: false`, `readOnlyHint` per tool. Results are returned as JSON text plus `structuredContent`.
