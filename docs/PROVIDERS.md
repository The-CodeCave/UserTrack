...
|---|---|---|---|---|---|---|
| **Better Auth** | users | Integration secret `ut_int_…` generated by UserTrack (shown once, rotatable); the founder installs `@usertrack/better-auth` and sets `USERTRACK_PROJECT_ID` / `USERTRACK_SECRET` | `POST <baseUrl>/usertrack/metrics` signed with HMAC-SHA256 (protocol v1; request + response signatures, ±5 min, nonce-bound reply): `totalUsers`, `newUsers` 24h/7d/30d, `daily[]` for history | totalUsers · createdUsers · historicalUsers | 30 daily `newUsers` in one request (`days: 30`) | Anonymous-plugin users excluded; retention not available (no session scan); adapters without `count` fall back to a bounded id scan (≤ 50k) and report `exactCounts: false`; requires plugin ≥ 0.1.0 / protocol 1 |
