...
2. Register it in `convex/providers/index.ts` and add the literal to `ProviderKind` (`types.ts`), `providerKind` (`convex/schema.ts`), `ProviderKind` in `convex/lib/integrationSetup.ts` and `PROVIDERS` in `src/lib/mcp/tools.ts`.
