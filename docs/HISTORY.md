...
1. `sync.startBackfill` writes a `backfills` row (`running`, `fromDay`, `toDay`, `trigger: first_sync | rolling | manual | rebuild`). A `rebuild` run first calls `sync.clearBackfilled`, which deletes the `backfilled` snapshots inside the window (see below).
...
**Rebuild — the one way back.** Because step 4 never rewrites a day, a source that was pointed at the wrong table writes a wrong curve *once* and keeps it forever: fixing the query afterwards changes the live total but not a single reconstructed day, and the chart keeps the step between them. `integrations.backfill({ rebuild: true })` closes that: `sync.clearBackfilled` deletes the snapshots marked `backfilled` inside the requested window, then the normal import runs and writes them again from the corrected source. Snapshots UserTrack observed live (`backfilled` unset) are never deleted, the daily rows are overwritten in place by `upsertDaily`, and the run is logged with `trigger: "rebuild"` so the sync log shows why the curve moved. Days outside the window keep whatever they had. The classic trigger is an event-log table (`growthEvents`, one `signup_completed` row per user) used as the users source: it only holds rows from the day it shipped, so the reconstruction is flat at ~0 until the day the SDK was installed — see the counting rules in `convex/lib/nativeSetup.ts`.
...
Founders can re-run it from the project page ("Backfill history", `integrations.backfill`, users role): **last 30 days**, **entire history** (`days: "all"` → the provider's reach; explicit day counts are clamped to it) or **rebuild history** (same reach, discards the reconstructed days first — behind a confirm, since it deletes data) and see the last ten runs (`integrations.backfills`). Charts and `GET /api/v1/saas/{slug}/history` expose `reconstructedUntil` (the last reconstructed day) and label those points "reconstructed from signup dates". MCP: `usertrack_sync_project` triggers a sync; a dedicated backfill tool is a roadmap item.
