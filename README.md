...
| **MCP** | `/mcp` — 50 tools for Claude Code, Cursor, Codex, VS Code or any MCP client: stack-aware provider recommendation (web + mobile), create project, setup instructions, configure + verify each source independently, activation / conversion setup, identity-mapping guidance, publish, metrics, history, rank + rank history, milestones, funnel + history, cohorts, trending, benchmarks + benchmark history, compare, share cards, embed code, founder profile, **discover**, **follow / unfollow products and founders**, **watchlist**, **datasets**, **webhooks** (list, create, update, test, deliveries). Scoped, hashed tokens (`follows:*`, `webhooks:*` scopes); audit trail. |
| **SEO** | Server-rendered pages, canonical URLs, JSON-LD (`SoftwareApplication`, `ItemList`, `Person`), intro + methodology + last-updated on every ranking page, monthly ranking archives, `sitemap.xml` (incl. frozen rankings), `robots.txt`, custom 404. Every public page has its own live OG/Twitter image (`next/og`, shared blueprint frame in `src/lib/og`): product pages render the logo, stats and 30-day curve, founder profiles their projects, every board its current top 4, plus cards for home, discover, categories, compare and developers — with a data-free fallback for the rest. |
