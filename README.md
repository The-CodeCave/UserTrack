...
| **Analytics** | Self-hosted, cookieless Rybbit (no banner, no session replay, IP hashed daily, token paths masked, `/api/**` `/embed/**` `/mcp` skipped). Typed catalog of 43 product events in `src/lib/analytics.ts` fired at their real call sites, pseudonymous identify on sign-in / clear on sign-out, server-side events from route handlers (`api_request`, `mcp_tool_called`, `badge_rendered`, `embed_rendered`, `native_event_ingested`) and Convex (`webhook_delivered`, `sync_completed`). Goals + funnels to create in the dashboard: `docs/ANALYTICS.md`. |
