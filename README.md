...
| **Embeds** | **Live widgets** (`/widget.js` loader → iframe `/embed/[slug]`: live user count, growth %, Verified by UserTrack, mini chart; auto/dark/light theme, 7d/30d, count-up, 5-minute refresh from `/api/embed/[slug].json`, every widget links back with `ref=embed` + UTM) and SVG badges (`/api/badge/[slug].svg`: users, growth, trending, verified, mini chart; dark/light, compact) with copy-paste snippets, per-IP rate limits, configurator at `/app/saas/[id]/embed`. Badge snippets link back with `ref=badge` + UTM. Embedding hosts are recorded for widgets and badges (domain only; best effort for badges) → “Embedded on N sites” with widget / badge labels on the dashboard and the public page. |
...
| **Analytics** | Self-hosted, cookieless Rybbit (no banner, no session replay, IP hashed daily, token paths masked, `/api/**` `/embed/**` `/mcp` skipped). Typed catalog of 44 product events in `src/lib/analytics.ts` fired at their real call sites, pseudonymous identify on sign-in / clear on sign-out, server-side events from route handlers (`api_request`, `mcp_tool_called`, `badge_rendered`, `embed_rendered`, `native_event_ingested`) and Convex (`webhook_delivered`, `sync_completed`). Outbound links (widgets, badges, share cards, X posts, MCP, emails) carry `ref` + UTM and sign-ups keep a sanitized first-touch source; goals + funnels to create in the dashboard: `docs/ANALYTICS.md`. |
...
| `node scripts/smoke.mjs [base] [mobile]` | E2E: sign-up → verify (Better Auth component adapter) → onboarding → publish → public page → dashboard (needs Chrome) |
| `node scripts/cache-check.mjs [base] [slug] [user]` | Hits every public route twice and asserts ISR `HIT` / `Cache-Control` (and that signed-in routes carry neither) |
| `node scripts/analytics-check.mjs [base]` | Prints every Rybbit call a page makes (page views, custom events, properties) |
| `node scripts/shots-ship1.mjs [public\|founder\|all]` | SHIP-1 launch pass: every public and signed-in launch surface at 375 / 768 / 1440 → `docs/screenshots/v1/ship/` |
...
| | `NEXT_PUBLIC_SENTRY_DSN` | optional | Sentry EU DSN. Missing → the SDK is neither initialised nor downloaded. `SENTRY_ORG` / `SENTRY_PROJECT` / `SENTRY_AUTH_TOKEN` only where source maps should be uploaded (CI) |
| | `NEXT_PUBLIC_APP_VERSION` | optional | Release name for `/api/health` and Sentry; falls back to `RAILWAY_GIT_COMMIT_SHA`, then `dev` |
...
## Operations
- **Health** — `GET /api/health` → `200 {ok, version, uptime}` with no Convex read (Railway's health check); `?deep=1` adds one Convex read plus the latest run of every background job and still answers 200 when Convex is down.
- **Background jobs** — every cron pages its table and writes a `jobRuns` row (`docs/ARCHITECTURE.md` → Background jobs).
- **Retention** — the daily sweep expires logs, counters and raw samples (`convex/retention.ts`); the periods are `RETENTION_DAYS` in `src/lib/legal.ts` and are exactly what `/privacy` states.
- **Errors** — `@sentry/nextjs` behind `NEXT_PUBLIC_SENTRY_DSN` (errors only, bodies / cookies / credentials / token parameters scrubbed), branded `error.tsx` / `global-error.tsx`, and public pages that render "Live data is temporarily unavailable" instead of crashing when Convex is unreachable.
- **CI** — `.github/workflows/ci.yml` runs lint, typecheck, tests, the workspace packages and a production build on every push and pull request, with no secrets.
...
src/lib/mcp/           server, tools (52) + setup workflow, config snippets + agent prompts
...
docs/                  ARCHITECTURE · API · MCP · PROVIDERS · FUNNEL · IDENTITY · METRICS · TRENDING · BENCHMARKS · DISCOVERY · FOLLOWS · HISTORY · DATASETS · WEBHOOKS ·
                       PROFILES · SHARING · SOCIAL · ANALYTICS · BACKLOG · ASSUMPTIONS · DEPLOYMENT · ROADMAP · CHANGELOG · CODECRAFT · RELEASE-v1.0 · DESIGN
...
`docs/screenshots/v1/ship/`; each feature ticket has its own folder next to it, and `api-checks.txt` holds the header /
rate-limit / cache transcript.

| | |
|---|---|
| ![Landing](docs/screenshots/v1/ship/landing-1440.png) **Landing** | ![Leaderboard](docs/screenshots/v1/ship/leaderboard-1440.png) **Leaderboard** |
| ![Discover](docs/screenshots/v1/ship/discover-1440.png) **Discover** | ![Product page](docs/screenshots/v1/ship/project-about-stack-1440.png) **Product page** (About + Company & stack) |
| ![Anonymous mode](docs/screenshots/v1/ship/project-anonymous-1440.png) **The same page in anonymous mode** | ![Founder profile](docs/screenshots/v1/ship/founder-1440.png) **Founder profile** |
| ![Dashboard](docs/screenshots/v1/ship/dashboard-1440.png) **Dashboard** | ![Settings](docs/screenshots/v1/ship/settings-1440.png) **Settings** (connected accounts, data & privacy) |
| ![Mobile landing](docs/screenshots/v1/ship/landing-375.png) **Landing at 375** | ![Mobile leaderboard](docs/screenshots/v1/ship/leaderboard-375.png) **Leaderboard at 375** |
