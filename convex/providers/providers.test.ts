...
describe("providers", () => {
  ...
  it("endpoint trust depends on domain match", () => {
    const v = providers.endpoint.validate({ url: "https://api.acme.com/users", token: "" });
  ...
  it("manual is never verified and validates counts", () => {
    expect(providers.manual.validate({ totalUsers: -1 }).ok).toBe(false);
    const v = providers.manual.validate({ totalUsers: 12.9 });
  ...
  it("clerk / supabase validation", () => {
    expect(providers.clerk.validate({ secretKey: "nope" }).ok).toBe(false);
    expect(providers.clerk.validate({ secretKey: "sk_test_abc" }).ok).toBe(true);
    expect(providers.supabase.validate({ url: "https://x.supabase.co", serviceKey: "k".repeat(30) }).ok).toBe(true);
    expect(providers.supabase.validate({ url: "https://x.com", serviceKey: "k".repeat(30) }).ok).toBe(false);
  });
});
