import { asCount, fetchJson, hostOf, type HistoryPoint, type Provider, type ProviderMetrics } from "./types";
...
export const posthog: Provider<PostHogConfig> = {
  ...
  capabilities: ["activation", "traffic", "history"],
  validate(c, role) {
  ...
  async fetch(cfg, role): Promise<ProviderMetrics> {
    if (role === "activation") {
      ...
      return { activatedUsers: first(all, "PostHog activated"), activated24h: first(d1, "PostHog activated"), activated7d: first(d7, "PostHog activated"), activated30d: first(d30, "PostHog activated") };
    }
