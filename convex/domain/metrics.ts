...
export function windowSummary(s: Doc<"saas">, timeframe: Timeframe) {
  const map = {
    "24h": { newUsers: s.newUsers24h, previous: s.newUsersPrev24h, activated: s.activated24h, growthPct: s.totalUsers - s.newUsers24h > 0 ? Math.round((s.newUsers24h / (s.totalUsers - s.newUsers24h)) * 1000) / 10 : 0, trendingScore: s.trendingScore24h },
    "7d": { newUsers: s.newUsers7d, previous: s.newUsersPrev7d, activated: s.activated7d, growthPct: s.growth7dPct ?? 0, trendingScore: s.trendingScore7d },
    "30d": { newUsers: s.newUsers30d, previous: s.newUsersPrev30d, activated: s.activated30d, growthPct: s.growth30dPct, trendingScore: s.trendingScore30d },
  }[timeframe];
  return {
    timeframe,
...
// Chart-ready series: raw snapshots for short ranges, daily rollups otherwise. `includeTraffic` respects the owner's public toggle.
export interface SeriesPoint { t: number; total: number; delta: number; activated?: number; visitors?: number }

export async function seriesFor(ctx: QueryCtx, s: Doc<"saas">, range: Range, includeTraffic = Boolean(s.showTraffic)): Promise<SeriesPoint[]> {
  ...
  return rows.map((r) => ({ t: Date.parse(`${r.day}T12:00:00Z`), total: r.totalUsers, delta: r.newUsers, activated: r.activatedUsers, visitors: includeTraffic ? r.visitors : undefined }));
}
...
export function shareData(s: Doc<"saas">, username?: string, milestones: Awaited<ReturnType<typeof milestonesFor>> = []) {
  ...
  return {
    ...
    headline: { totalUsers: s.totalUsers, newUsers30d: s.newUsers30d, growth30dPct: s.growth30dPct, rank: s.rank, trendingRank: s.trendingRank },
