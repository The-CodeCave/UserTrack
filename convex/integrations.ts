...
import { candidateBaseUrls } from "./providers/native";
import { type NativeSource, normalizeSource } from "./lib/nativeProtocol";
...
export type TestResult =
  | { ok: true; detected?: number; metrics: ProviderMetrics; trust: Trust; verification: VerificationLevel; capabilities: ProviderCapabilities; durationMs: number; publicConfig: Record<string, string>; discoveredUrl?: string }
  | { ok: false; error: string; retryable: boolean; tried?: string[] };
...
// The native base URL is derived, never typed, so a failed verify usually means the app runs on a different origin than
// the marketing site. Probe the plausible ones in parallel and keep the first that answers with a valid signature.
async function discoverNativeUrl(ctx: ActionCtx, integration: Doc<"integrations">, websiteUrl: string, role: Role) {
  const cfg = integration.config as { url: string; source?: NativeSource };
  const candidates = candidateBaseUrls(cfg.url, websiteUrl, normalizeSource(cfg.source));
  if (!candidates.length) return { candidates, found: null };
  const runs = await Promise.allSettled(candidates.map((url) => fetchMetrics(ctx, integration.provider, { ...integration.config, url }, role)));
  const hit = runs.findIndex((r) => r.status === "fulfilled");
  if (hit === -1) return { candidates, found: null };
  await ctx.runMutation(internal.integrations.adoptBaseUrl, { integrationId: integration._id, url: candidates[hit] });
  return { candidates, found: { url: candidates[hit], metrics: (runs[hit] as PromiseFulfilledResult<ProviderMetrics>).value } };
}

// Adopting a probed URL rewrites only the base URL; the credential and every other field stay untouched.
export const adoptBaseUrl = internalMutation({
  args: { integrationId: v.id("integrations"), url: v.string() },
  handler: async (ctx, { integrationId, url }) => {
    const i = await ctx.db.get(integrationId);
    if (!i) return;
    const config = { ...(i.config as Record<string, unknown>), url };
    await ctx.db.patch(integrationId, { config, publicConfig: getProvider(i.provider).publicConfig(config) });
  },
});
...
export const verifyStored = action({
  ...
  handler: async (ctx, { saasId, role: rawRole }): Promise<TestResult> => {
    ...
    const started = Date.now();
    const succeed = (metrics: ProviderMetrics, config: unknown, discoveredUrl?: string): Extract<TestResult, { ok: true }> => {
      const trust = p.trust(config, websiteUrl);
      const capabilities = describeProvider(p, config, role as Role);
      const safe = { ...metrics };
      delete safe.identities;
      return { ok: true as const, detected: detectedCount(metrics, role), metrics: safe, trust, verification: verificationLevel(integration.provider, trust, capabilities, role as Role), capabilities, durationMs: Date.now() - started, publicConfig: p.publicConfig(config), discoveredUrl };
    };
    try {
      ...
      return { ...succeed(metrics, integration.config), publicConfig: integration.publicConfig ?? p.publicConfig(integration.config) };
    } catch (e) {
      const err = e as Error;
      if (integration.provider === "native" || integration.provider === "better_auth") {
        const { candidates, found } = await discoverNativeUrl(ctx, integration, websiteUrl, role as Role);
        if (found) {
          await ctx.runMutation(internal.integrations.markVerified, { integrationId: integration._id });
          return succeed(found.metrics, { ...integration.config, url: found.url }, found.url);
        }
        ...
        return { ok: false as const, error: err.message.slice(0, 300), retryable: err instanceof ProviderError ? err.retryable : true, tried: candidates };
      }
      await ctx.runMutation(internal.integrations.recordVerifyFailure, { integrationId: integration._id, error: err.message.slice(0, 300) });
      return { ok: false as const, error: err.message.slice(0, 300), retryable: err instanceof ProviderError ? err.retryable : true };
    }
