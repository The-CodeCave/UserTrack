...
import { ENV_EXAMPLE_SNIPPET, PLUGIN_SNIPPET, betterAuthSetup } from "./betterAuthSetup";
...
export const INTEGRATION_CATALOG: CatalogEntry[] = [
  {
    ...
    roles: ["users"],
    trust: "verified",
    summary: "Official plugin (@usertrack/better-auth, ~2 min): the app exposes signed aggregate metrics (total users, signups per window, daily history) from its own Better Auth database. No PII, no database credentials shared with UserTrack.",
    detects: ["better-auth", "@better-auth/core", "@better-auth/cli", "BETTER_AUTH_SECRET", "BETTER_AUTH_URL", "@convex-dev/better-auth"],
    credentials: [
      { key: "url", label: "Better Auth base URL", secret: false, whereToFind: "The public base URL of the app's Better Auth instance, e.g. https://app.example.com/api/auth (baseURL + basePath). Defaults to <websiteUrl>/api/auth.", envVarHints: ["BETTER_AUTH_URL", "NEXT_PUBLIC_APP_URL"], format: "https://…/api/auth" },
    ...
    permissions: ["No third-party credential is needed: the plugin runs inside the app and answers signed requests with aggregate counts only.", "The secret authenticates both directions (UserTrack → plugin pull, plugin → UserTrack events); rotate it from the dashboard or usertrack_create_integration { rotate: true }."],
    reads: "POST <baseUrl>/usertrack/metrics (HMAC-signed): totalUsers, newUsers 24h/7d/30d, optional daily series (≤ 90 days). Optional pushed events user.created / user.deleted with a pseudonymous subject. Never user records.",
    neverSent: NEVER,
  },
  {
    provider: "clerk",
  ...
  },
  {
    ...
    detects: ["next-auth", "@auth/core", "lucia", "convex", "@prisma/client", "drizzle-orm", "mongoose", "pg", "mysql2", "kysely", "sequelize", "typeorm", "amplitude", "@amplitude/analytics-browser", "mixpanel", "mixpanel-browser"],
    credentials: [
...
const DETECTION_ALIASES: Record<string, ProviderKind> = {
  ...
  "convex-dev-better-auth": "better_auth",
  nextauth: "endpoint",
  "next-auth": "endpoint",
  "auth.js": "endpoint",
  authjs: "endpoint",
...
const USERS_PRIORITY: ProviderKind[] = ["better_auth", "supabase", "clerk", "firebase", "auth0", "postgres", "endpoint", "manual"];
...
export function recommendIntegrations(input: RecommendInput) {
  ...
  for (const k of USERS_PRIORITY) {
    if (kinds.has(k) && k !== "manual") {
      users = k;
      reasoning.push(k === "better_auth" ? `Better Auth detected (${detected.filter((d) => d.provider === k).map((d) => d.raw).join(", ")}) — an official plugin exists: install @usertrack/better-auth, add userTrack() to the plugins array and UserTrack pulls signed aggregate metrics (no PII, no database credentials). Use usertrack_get_better_auth_setup.` : `${catalogEntry(k)!.label} detected (${detected.filter((d) => d.provider === k).map((d) => d.raw).join(", ")}) — it can report user counts directly with a read-only credential.`);
      break;
    }
  }
...
const IDENTITY_FIELD: Record<string, string> = { better_auth: "better-auth user.id (session.user.id)", firebase: "firebase.uid", supabase: "supabase.auth.users.id", clerk: "clerk.userId", auth0: "auth0.user_id", postgres: "users.<idColumn>", endpoint: "identities.signedUp[].id (your user id)" };
...
export function integrationSetup(input: { provider: string; role?: Role; framework?: string; detectedProviders?: readonly string[]; websiteUrl?: string; projectId?: string }) {
  ...
  if (entry.provider === "better_auth") {
    steps.splice(0, steps.length);
    for (const s of betterAuthSetup({ projectId: input.projectId, packageManager: input.framework, framework: input.framework }).steps) steps.push({ id: `better_auth:${s.id}`, title: s.title, detail: s.detail, action: s.action, tool: s.tool });
    codeTemplates.push({ framework: "any", path: "lib/auth.ts (your Better Auth config)", language: "typescript", code: PLUGIN_SNIPPET }, { framework: "any", path: ".env.example", language: "dotenv", code: ENV_EXAMPLE_SNIPPET });
  }
  ...
  if (entry.provider !== "better_auth") steps.push(
  ...
  return {
    ...
    nextTool: entry.provider === "better_auth" ? "usertrack_get_better_auth_setup" : "usertrack_configure_integration",
