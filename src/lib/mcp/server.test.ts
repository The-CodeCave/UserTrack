...
describe("handleMcpRequest", () => {
  ...
  it("lists all 29 tools with input schemas", async () => {
  ...
  it("serves the onboarding prompt", async () => {
    ...
    expect(list.result.prompts.map((p: { name: string }) => p.name)).toEqual(["add_project_to_usertrack", "add_mobile_app_to_usertrack", "add_better_auth_project_to_usertrack"]);
...
describe("/mcp route", () => {
  ...
  it("serves JSON-RPC for a valid ut_mcp_ token and adds CORS headers", async () => {
    ...
    const body = await res.json();
    expect((Array.isArray(body) ? body[0] : body).result.tools).toHaveLength(29);
  });
