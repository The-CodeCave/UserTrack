  ...
  return {
    ...
    components: {
      ...
      schemas: {
        ...
        Profile: { type: "object", properties: { username: { type: "string" }, displayName: { type: "string" }, avatarUrl: { type: "string" }, bio: { type: "string" }, location: { type: "string" }, links: { type: "object" }, xState: { type: "string", enum: ["connected_via_oauth", "handle_provided", "unavailable"] }, followers: { type: "integer" }, joinedAt: { type: "string", format: "date-time" }, urls: { type: "object", properties: { profile: { type: "string" }, card: { type: "string" }, history: { type: "string" } } }, metrics: { type: "object", description: "Aggregates over public projects only; activation is weighted (sum activated / sum users of projects with an activation source)", properties: { projects: { type: "integer" }, verifiedProjects: { type: "integer" }, totalUsers: { type: "integer" }, newUsers7d: { type: "integer" }, newUsers30d: { type: "integer" }, growth30dPct: { type: "number" }, changeVsPrev30dPct: { type: "number" }, activation: { type: "object" }, convertedUsers: { type: "integer" }, bestRank: { type: "integer" }, trendingProjects: { type: "integer" }, biggestGrowth: { type: "object" } } }, saas: { type: "array", items: { $ref: "#/components/schemas/Saas" } } } },
