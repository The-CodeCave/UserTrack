...
describe("availableShareKinds", () => {
  it("hides kinds without data", () => {
    ...
    const all = availableShareKinds({ ...s, rank: 4, trendingRank: 2, activationRatePct: 12.5 });
    expect(all.map((k) => k.kind)).toEqual(["users", "growth", "week", "rank", "trending", "activation"]);
    expect(all.map((k) => k.label)).toEqual(["12.5K users", "+1,900 in 30d", "+120 this week", "#4 on UserTrack", "#2 trending", "13% activation"]);
  });
});
