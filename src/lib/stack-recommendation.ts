...
export interface StackRecommendation { users: ProviderKind | null; activation?: ProviderKind; traffic?: ProviderKind; conversion?: ProviderKind; notes: string[] }
export interface Option { value: string; label: string }
export interface StackQuestion { key: StackKey; title: string; options: Option[] }

const IDENTITY: Record<string, string> = { clerk: "Clerk", supabase: "Supabase", firebase: "Firebase", better_auth: "Better Auth", auth0: "Auth0", postgres: "PostgreSQL", custom: "Custom Backend", other: "Other" };
...
export function stackQuestions(platform: Platform): StackQuestion[] {
  ...
  const identity: StackQuestion = { key: "identity", title: "How do users sign in?", options: pick(IDENTITY, mobile ? ["firebase", "supabase", "auth0", "custom", "other"] : ["clerk", "supabase", "firebase", "better_auth", "auth0", "postgres", "other"]) };
...
export function recommendStack(a: StackAnswers): StackRecommendation {
  ...
  if (a.authMethods?.includes("apple")) {
    const source = a.identity && IDENTITY_PROVIDER[a.identity] !== "endpoint" ? IDENTITY[a.identity] : "your own backend";
