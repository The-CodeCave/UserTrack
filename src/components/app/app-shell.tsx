...
import { LayoutGrid, Boxes, UserRound, Settings, LogOut } from "lucide-react";
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col md:flex-row">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-line bg-background/90 backdrop-blur md:hidden">
        {NAV.map(({ href, label, Icon }) => {
          const active = href === "/app" ? pathname === href : pathname.startsWith(href);
          return (
            <Link key={href} href={href} className={cn("flex h-14 flex-col items-center justify-center gap-1 text-[10px] font-mono uppercase tracking-wider", active ? "text-pink" : "text-muted-foreground")}>
