// Inbound lifecycle events from @usertrack/better-auth. Thin proxy: the signature is verified inside Convex with the
// integration secret; this route only forwards the raw body and the signature headers (never logs them).
import { fetchAction } from "convex/nextjs";
import { api } from "@convex/_generated/api";
import { HEADER_NONCE, HEADER_PROJECT, HEADER_SIGNATURE, HEADER_TIMESTAMP } from "@convex/lib/betterAuthProtocol";
...
export async function POST(req: Request) {
  ...
  const rl = take(`ba-events:${projectId}`, Date.now(), 600);
  ...
  const r = await fetchAction(api.betterAuth.ingestEvent, {
...
export function GET() {
  return Response.json({ error: "POST signed lifecycle events here (see https://usertrack.dev/developers/integrations/better-auth)" }, { status: 405 });
}
