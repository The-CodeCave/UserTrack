  ...
  return (
    ...
    >
      <section lang="de" className="border border-line bg-card/80 p-4 sm:p-5">
        ...
        <p><strong>Betroffenenrechte:</strong> Sie haben das Recht auf Auskunft (Art. 15 DSGVO), Berichtigung (Art. 16), Löschung (Art. 17), Einschränkung der Verarbeitung (Art. 18), Datenübertragbarkeit (Art. 20) und Widerspruch (Art. 21), sowie das Recht, eine erteilte Einwilligung jederzeit mit Wirkung für die Zukunft zu widerrufen (Art. 7 Abs. 3). Konto löschen und Daten exportieren können Sie selbst unter <a href="/app/settings">/app/settings</a>; alle anderen Anfragen richten Sie an {OPERATOR.email}. Beschwerden nehmen die {SUPERVISORY_AUTHORITY.name} ({SUPERVISORY_AUTHORITY.address}) oder jede andere Aufsichtsbehörde entgegen.</p>
      ...
      <Section id="cookies" title="7. Cookies">
        <p>UserTrack uses <strong>only strictly necessary cookies</strong>: the Better Auth session cookie that keeps you signed in (and its short-lived CSRF / state cookies during sign-in with a provider). They are required to provide the service you asked for (§ 25(2) no. 2 TDDDG), so <strong>no consent banner is shown</strong>. There are no advertising, tracking or third-party cookies, and embeds and badges set no cookies on the sites that host them. Local storage is used only for interface preferences such as chart ranges.</p>
      </Section>

      <Section id="analytics" title="8. Analytics">
        <p>We measure page views with <strong>Rybbit</strong>, an open-source analytics tool that we host ourselves on EU infrastructure operated by {OPERATOR.name}. It is <strong>cookieless</strong>, sets no identifier on your device, performs no cross-site tracking and does not use session replay. Your IP address is used only to derive a daily-rotating, salted hash for counting unique visitors and is never stored; coarse location (country / region), browser family, referrer and page path are aggregated. Because no information is stored on or read from your device, no consent is required (Art. 6(1)(f) GDPR). If you still prefer not to be counted, set <code className="font-mono text-foreground">localStorage.setItem(&quot;disable-rybbit&quot;, &quot;1&quot;)</code> in your browser.</p>
      </Section>
